...
export default function BusinessPopup({ biz, onClose }) {
  ...
  const scoreColor = biz.score >= 71 ? "#121830" : biz.score >= 41 ? "#f5a623" : "#4a9eff";
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#FFFFFF", border: "1px solid #D7DAE1", borderRadius: 10, width: "100%", maxWidth: 540, maxHeight: "90vh", overflowY: "auto", padding: 24 }}>

        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#121830", marginBottom: 4 }}>{biz.name}</div>
        ...
        {sourceList.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: "monospace", fontSize: 9, color: "#6B7280", letterSpacing: 2, marginBottom: 6 }}>DATA SOURCES</div>
        ...
        {/* Notes */}
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontFamily: "monospace", fontSize: 9, color: "#6B7280", letterSpacing: 2, marginBottom: 6 }}>NOTES</div>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="Add notes about this business..."
            style={{ width: "100%", background: "#F5F6F8", border: "1px solid #D7DAE1", borderRadius: 5, color: "#121830", padding: "10px 12px", fontSize: 13, fontFamily: "Inter, sans-serif", resize: "vertical", minHeight: 80 }}
          />
        </div>
        ...
        {saved && (
          <div style={{ fontFamily: "monospace", fontSize: 11, color: "#22c55e", marginBottom: 12, background: "#EAFBF1", padding: "8px 12px", borderRadius: 4, border: "1px solid #22c55e33" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setEditing(!editing)} style={{ flex: 1, background: editing ? "#E2E4E9" : "transparent", border: "1px solid #D7DAE1", color: editing ? "#121830" : "#888", borderRadius: 5, padding: "10px", fontFamily: "monospace", fontSize: 11, cursor: "pointer", letterSpacing: 1 }}>
            {editing ? "SAVE EDITS" : "EDIT INFO"}
          </button>
          <button onClick={handleSaveToCRM} disabled={saving || saved} style={{ flex: 2, background: saved ? "#22c55e" : "#121830", border: "none", color: "#fff", borderRadius: 5, padding: "10px", fontFamily: "monospace", fontSize: 11, fontWeight: 700, cursor: saving || saved ? "not-allowed" : "pointer", letterSpacing: 1, opacity: saving ? 0.7 : 1 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
        <div style={{ fontFamily: "monospace", fontSize: 9, color: "#6B7280", letterSpacing: 2 }}>{label}</div>
        {conflicted && <span style={{ fontFamily: "monospace", fontSize: 9, color: "#f5a623" }}>⚠</span>}
      </div>
      {editing && onChange ? (
        <input value={value || ""} onChange={e => onChange(e.target.value)}
          style={{ width: "100%", background: "#F5F6F8", border: conflicted ? "1px solid #f5a62366" : "1px solid #12183066", borderRadius: 4, color: "#121830", padding: "7px 10px", fontSize: 13, fontFamily: "Inter, sans-serif" }} />
      ) : (
        <div style={{ fontSize: 13, color: value ? (isLink ? "#4a9eff" : conflicted ? "#f5a623" : "#121830") : "#9AA0AC" }}>
          {isLink && value
