      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={fetchCRMLeads} style={{ background: "#E2E4E9", border: "1px solid #D7DAE1", color: "#888", borderRadius: 5, padding: "6px 14px", fontFamily: "monospace", fontSize: 11, cursor: "pointer" }}>↻ REFRESH</button>
          </div>
        </div>
      </div>

      {loading ? (
        <div style={{ fontFamily: "monospace", fontSize: 13, color: "#6B7280", textAlign: "center", padding: 60 }}>Loading CRM...</div>
      ) : leads.length === 0 ? (
        ...
        <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 16 }}>
          {CRM_STAGES.map(stage => (
            <div key={stage.key} style={{ minWidth: 220, flex: "0 0 220px" }}>
              {/* Column Header */}
              <div style={{ background: "#FFFFFF", border: "1px solid #E2E4E9", borderTop: `3px solid ${stage.color}`, borderRadius: 6, padding: "10px 14px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {getStageLeads(stage.key).map(lead => (
                  <div key={lead.id} style={{ background: "#FFFFFF", border: "1px solid #E2E4E9", borderLeft: `3px solid ${scoreColor(lead.score)}`, borderRadius: 6, padding: 12 }}>
                    <div onClick={() => setSelected(lead)} style={{ cursor: "pointer", marginBottom: 10 }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                        ...
                        {lead.assigned_agent_name && (
                          <div style={{ fontFamily: "monospace", fontSize: 9, color: "#6B7280" }}>👤 {lead.assigned_agent_name}</div>
                        )}
                      </div>
                    </div>
                    ...
                    <select
                      ...
                      style={{ width: "100%", marginBottom: 8, background: "#F5F6F8", border: "1px solid #D7DAE1", color: "#888", borderRadius: 3, padding: "4px 6px", fontFamily: "monospace", fontSize: 9 }}
                    >
                ...
                {getStageLeads(stage.key).length === 0 && (
                  <div style={{ border: "1px dashed #E2E4E9", borderRadius: 6, padding: "20px 12px", textAlign: "center", fontFamily: "monospace", fontSize: 10, color: "#D7DAE1" }}>Empty</div>
                )}
              </div>
            </div>
          ))}
